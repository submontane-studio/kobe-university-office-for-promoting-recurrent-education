...
program
  ...
  .option("-f, --avif", "avif化を行う", false)
  .option("-s, --avif-suffix-add", "avif化の際、拡張子を書き換え（false）するか追加（true）するか", false)
...
const imageFileList = [];
globSync(`${IMAGE_DIR}/**/*.*`).map(file => {
  // windows対応
  const normalizedFile = `./${file.replace(/\\/g, "/")}`;
  imageFileList.push(normalizedFile.replace(IMAGE_DIR, "."));
});
